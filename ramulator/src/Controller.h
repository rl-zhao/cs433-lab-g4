...
public:
    ...
    /* ======= BLISS state (Task 5) ======= */
    // Last app (core) we issued a request for, and how many consecutive issues.
    int  bliss_last_app   = -1;
    int  bliss_consec     = 0;
    // Blacklist flags per app; 1=blacklisted, 0=not.
    std::vector<uint8_t> bliss_blacklisted;
    // When did we last clear the blacklist?
    long bliss_last_clear = 0;
    // Tunables (defaults per BLISS): can be overridden via config.
    int  bliss_threshold  = 4;        // "Blacklisting Threshold"
    long bliss_interval   = 10000;    // "Clearing Interval" (cycles)

    inline bool is_app_blacklisted(int app) const {
      return app >= 0 && app < (int)bliss_blacklisted.size() && bliss_blacklisted[app];
    }
    inline void bliss_clear_blacklist() {
      std::fill(bliss_blacklisted.begin(), bliss_blacklisted.end(), uint8_t(0));
    }
        ...
        cmd_trace_files(channel->children.size())
    {
        ...
        write_req_queue_length_avg
            ...
            ;
        /* ======= BLISS init (Task 5) ======= */
        // Size blacklist by number of cores; leave all clear initially.
        bliss_blacklisted.assign(configs.get_core_num(), uint8_t(0));
        // Optional overrides from config:
        if (configs.contains("bliss_blacklisting_threshold")) {
          bliss_threshold = std::stoi(configs["bliss_blacklisting_threshold"]);
        }
        if (configs.contains("bliss_clearing_interval")) {
          bliss_interval = std::stol(configs["bliss_clearing_interval"]);
        }
    ...
    {
        clk++;
        // BLISS: periodically clear blacklist (independent of queue state)
        if (scheduler->type == Scheduler<T>::Type::BLISS) {
            if (clk - bliss_last_clear >= bliss_interval) {
                bliss_clear_blacklist();
                bliss_last_clear = clk;
                bliss_consec = 0;
                bliss_last_app = -1;
            }
        }
        ...
        if (req->is_first_command) {
            // cs433: This is what executes when a new request has been selected for scheduling
            // For BLISS, this is where you want to determine how many requests a core has issued
            // You may need to reset it somewhere else

            // ======= BLISS: update consecutive-issue counter & blacklist if needed =======
            if (scheduler->type == Scheduler<T>::Type::BLISS) {
                int app = req->coreid;
                if (bliss_last_app == app) {
                    ++bliss_consec;
                } else {
                    bliss_last_app = app;
                    bliss_consec   = 1;
                }
                // "Exceeds" the threshold => blacklist this app; then reset the run.
                if (bliss_consec > bliss_threshold && app >= 0 && app < (int)bliss_blacklisted.size()) {
                    bliss_blacklisted[app] = 1;
                    bliss_last_app = -1;
                    bliss_consec   = 0;
                }
            }
