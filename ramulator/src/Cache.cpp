...
namespace ramulator
{
inline bool Cache::way_allowed(int coreid, int way) const {
  if (!is_last_level) return true;
  if (cachesys->way_masks.empty()) return true;
  uint64_t mask = cachesys->way_masks[coreid % cachesys->way_masks.size()];
  return (way >= 0) && (way < (int)assoc) && ((mask >> way) & 1ULL);
}
...
bool Cache::send(Request req) {
  ...
  if(cachesys->cache_qos == CacheSystem::Cache_QoS::way_partitioning) {  
    ...
    if (is_hit(lines, req.addr, &line)) {
      ...
      std::list<Line>::iterator newline;
      if (is_last_level) {
        // Enforce way partitioning only at LLC
        // Assumes Request carries a coreid (rename if your field differs).
        newline = allocate_line_wp(lines, req.addr, req.coreid);
      } else {
        newline = allocate_line(lines, req.addr);
      }
...
void Cache::evictline(long addr, bool dirty) {
  ...
  if(cachesys->cache_qos == CacheSystem::Cache_QoS::way_partitioning) {  
    ...
    lines.push_back(Line(addr, get_tag(addr), false,
        dirty || line->dirty, line->way, line->owner_core));
    lines.erase(line);
  }
  // cs433 QoS: Custom QoS
  else if(cachesys->cache_qos == CacheSystem::Cache_QoS::custom) {  
...
// Way-partitioned allocation: honor per-core way masks at LLC.
std::list<Cache::Line>::iterator Cache::allocate_line_wp(
    std::list<Line>& lines, long addr, int coreid) {
  // Make sure default masks exist if config didn’t provide any yet.
  cachesys->ensure_masks_initialized((int)assoc);
  // 1) Build used-way map for this set.
  std::vector<bool> used(assoc, false);
  for (const auto& l : lines) {
    if (l.way >= 0 && l.way < (int)assoc) used[l.way] = true;
  }
  // 2) Try to find a FREE allowed way for this core.
  int chosen_way = -1;
  for (int w = 0; w < (int)assoc; ++w) {
    if (!used[w] && way_allowed(coreid, w)) { chosen_way = w; break; }
  }
  if (chosen_way >= 0) {
    // Allocate MRU line bound to chosen_way.
    lines.push_back(Line(addr, get_tag(addr), /*lock=*/true, /*dirty=*/false,
                         chosen_way, coreid));
    auto it = lines.end(); --it;
    return it;
  }
  // 3) Need eviction among ALLOWED ways only: pick LRU that is unlocked at all levels.
  auto victim = find_if(lines.begin(), lines.end(),
      [this, coreid](Line line) {
        if (!way_allowed(coreid, line.way)) return false;
        bool ok = !line.lock;
        if (!is_first_level) {
          for (auto hc : higher_cache) {
            if (!ok) return false;
            ok = ok && hc->check_unlock(line.addr);
          }
        }
        return ok;
      });
  if (victim == lines.end()) {
    // No unlocked allowed victim: stall.
    return victim;
  }
  int victim_way = victim->way;
  evict(&lines, victim);
  // 4) Install the new line in the just-freed allowed way.
  lines.push_back(Line(addr, get_tag(addr), /*lock=*/true, /*dirty=*/false,
                       victim_way, coreid));
  auto it = lines.end(); --it;
  return it;
