...
public:
    ...
    {
        // Simple reduce with comparator for these types
        if (type == Type::FCFS || type == Type::FCFSBank || type == Type::BLISS || type == Type::Custom) { // cs433
...
private:
    ...
    function<ReqIter(ReqIter, ReqIter)> compare[int(Type::MAX)] = {
        ...
        // cs433: Add your BLISS scheduler comparison here
        // BLISS
        [this] (ReqIter req1, ReqIter req2) {
            // cs433: ADD CODE BELOW THIS LINE
            //
            // SOME TIPS
            ...
            // 1) Prioritize non-blacklisted apps over blacklisted apps
            bool bl1 = this->ctrl->is_app_blacklisted(req1->coreid);
            bool bl2 = this->ctrl->is_app_blacklisted(req2->coreid);
            if (bl1 ^ bl2) {
                // Return the non-blacklisted request
                return bl1 ? req2 : req1;
            }
            // 2) Among equal blacklist status, prefer row-buffer hits that are ready
            ...
            // 3) Tie-breaker: older request first
            return (req1->arrive <= req2->arrive) ? req1 : req2;
            // cs433: ADD CODE ABOVE THIS LINE
        },

        // cs433: Add your Custom scheduler comparison here
        // Custom
        [this] (ReqIter req1, ReqIter req2) {
            // cs433: ADD CODE BELOW THIS LINE
            bool ready1 = this->ctrl->is_ready(req1) && this->ctrl->is_row_hit(req1);
