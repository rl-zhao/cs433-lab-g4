...
public:
    ...
    // cs433
    bool is_way_partitioning() const {
      if (options.find("cacheqos") != options.end()) 
      {
        const std::string& qos_option = (options.find("cacheqos"))->second;
        return (qos_option == "way_partitioning");
      }
      return false;
    }
